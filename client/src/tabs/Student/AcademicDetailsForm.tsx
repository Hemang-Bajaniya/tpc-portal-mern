...
export default function AcademicDetailsForm() {
  ...
  const validateField = (name: string, value: string | File | null) => {
    ...
    if ("required" in rule && rule.required && !value) return `${name.replace("_", " ")} is required`;
    if (value && typeof value === 'string' && (!rule.regex.test(value) || ("min" in rule && Number(value) < rule.min) || ("max" in rule && Number(value) > rule.max))) {
      return rule.message;
    }
    return "";
  };
  ...
  const handleSemChange = (index: number, value: string) => {
    ...
    const semesterErrors = validateField("semesters", null) as { [key: number]: { sgpa?: string } };
    setErrors((e) => ({ ...e, semesters: semesterErrors }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const fieldsToValidate: (keyof Omit<typeof form, "semesters" | "qualificationType" | "hsc_percentage" | "diploma_cgpa">)[] = ["ssc_percentage", "be_cgpa", "liveKT", "deadKT"];

    fieldsToValidate.forEach((field) => {
      const error = validateField(field, form[field]);
      if (error) {
        newErrors[field] = error as string;
        hasErrors = true;
      }
    });

    if (form.qualificationType === "HSC") {
      const error = validateField("hsc_percentage", form.hsc_percentage);
      if (error) {
        newErrors.hsc_percentage = error.toString(); //changed
        hasErrors = true;
      }
    } else if (form.qualificationType === "Diploma") {
      const error = validateField("diploma_cgpa", form.diploma_cgpa);
      if (error) {
        newErrors.diploma_cgpa = error.toString(); //changed
        hasErrors = true;
      }
    }

    const semesterErrors = validateField("semesters", null) as { [key: number]: { sgpa?: string } };
    if (Object.keys(semesterErrors).length > 0) {
      newErrors.semesters = semesterErrors;
      hasErrors = true;
    }
