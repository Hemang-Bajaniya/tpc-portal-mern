...
export const listPendingStudents = async (req, res) => {
    try {
        ...
        const tpcProfile = await TpcProfile.findOne({ userId: tpcUserId }).populate("dept_id");

        if (!tpcProfile) return res.status(404).json(apiResponse({ success: false, message: 'TPC profile not found', status: 404 }));
        ...
        const pendingStudents = await User.find(filter)
            ...
            .limit(pageSize).populate("dept_id");
        ...
        res.status(500).json(apiResponse({ success: false, message: 'Server error', error: err.message, status: 500 }));
    }
};
...
export const approveOrDeclineStudent = async (req, res) => {
    try {
        const tpcUserId = req.user.userId;
        const tpcProfile = await TpcProfile.findOne({ userId: tpcUserId }).populate("dept_id");
        if (!tpcProfile) return res.status(404).json({ message: 'TPC profile not found' });
        const deptId = tpcProfile.dept_id._id;
        const { studentId, approve } = req.body;
        const student = await User.findById(studentId);
        if (!student) return res.status(404).json({ message: 'Student not found' });
        if (String(student.dept_id) !== String(deptId)) {
            return res.status(403).json({ message: 'Not authorized to approve/decline this student' });
        }
...
export const getTpcProfile = async (req, res) => {
    try {
        ...
        if (!tpcProfile) {
            return res.status(404).json(apiResponse({ success: false, message: 'TPC profile not found', status: 404 }));
        }
        return res.json(apiResponse({
            data: tpcProfile,
